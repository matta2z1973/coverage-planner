...
export default async function MyRequestsPage() {
  ...
  const requests = await db
    ...
    .where(eq(coverageRequests.createdBy, user.id))
    .orderBy(desc(coverageRequests.createdAt));

  const requestIds = requests.map((r) => r.id);

  // Compute per-request stats: total slots, claimed slots, earliest date.
  const stats =
    requestIds.length > 0
      ? await db
          .select({
    ...
    <main className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-6 py-12">
      <header>
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Coverage requests you&rsquo;ve posted.
        </p>
      </header>

      {requests.length === 0 ? (
        <section className="rounded-lg border border-zinc-200 p-5 text-sm dark:border-zinc-800">
          You haven&rsquo;t posted any coverage requests yet.{" "}
          <Link
            href="/requests/new"
                ...
                <li key={r.id} className="px-5 py-3">
                  ...
                  >
                    ...
                    <span className="ml-auto text-xs text-zinc-600 dark:text-zinc-400">
                      {claimed}/{total} claimed{open > 0 ? ` · ${open} open` : ""}
                    </span>
                  </Link>
                </li>
              );
